<div align="center">
  <h1 align="center">Fantasy Layers</h1>
  <p align="center">地图图层操作库，集成Webpack、Gulp</p>
...
```bash
location ^~/fantasy-layers/{
	try_files $uri $uri/ /fantasy-layers/index.html;
}
```

通过如下url对页面进行访问：

http://yourHostname/fantasy-layers/
...
```bash
location ^~/fantasy-layers/{
	alias yourPhysicalPath\\fantasy-layers\\dist\\;
	try_files $uri $uri/ /fantasy-layers/index.html;
}
```
...
访问 http://localhost:8070/fantasy-layers/ 查看页面
